...
mod LizInventory {
    ...
    impl LizInventoryImpl of super::ILizInventory<ContractState> {
        fn add_stock(ref self: ContractState, product: felt252, new_stock: u32 ) {
            ...
            let mut existingStock = self.inventory.read(product);
            existingStock += new_stock;
            self.inventory.write(product, existingStock);
        }

        fn purchase(ref self: ContractState, product: felt252, quantity: u32) {
            let mut existingQuantity = self.inventory.read(product);
            existingQuantity -= quantity;
            self.inventory.write(product, existingQuantity);
        }
